...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    ...
    if (user.organizationId !== undefined) { values.organizationId = user.organizationId; updateSet.organizationId = user.organizationId; }
    if (user.orgRole !== undefined) { values.orgRole = user.orgRole; updateSet.orgRole = user.orgRole; }
  ...
  try {
    return await db
      .select({
        ...
        approvalStatus: teamAbsences.approvalStatus,
...
export async function getProjectBurnRate(projectId: number) {
  ...
  // Use contractAmount (contracted fee) for each consultant contract — deducted from budget
  // regardless of payment status, per business rules.
  const contracts = await db.select({ contractAmount: consultantContracts.contractAmount }).from(consultantContracts).where(eq(consultantContracts.projectId, projectId));
  const consultantCost = contracts.reduce((s, c) => s + (c.contractAmount ?? 0), 0);
...
> {
  ...
  // 3. Consultant contracted fees per project — use contractAmount regardless of payment status.
  // This ensures the full consultant obligation is reflected in the budget from day one.
  const contractRows = await db
    .select({ projectId: consultantContracts.projectId, contractAmount: consultantContracts.contractAmount })
  ...
  for (const c of contractRows) {
    consultantByProject.set(
      c.projectId,
      (consultantByProject.get(c.projectId) ?? 0) + (c.contractAmount ?? 0),
    );
  }
...
export async function getUserStripeCustomerId(userId: number): Promise<string | null> {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select({ stripeCustomerId: users.stripeCustomerId }).from(users).where(eq(users.id, userId)).limit(1);
  return result.length > 0 ? (result[0].stripeCustomerId ?? null) : null;
}
...
export async function updateSubscriptionStatus(stripeSubscriptionId: string, status: "active" | "canceled" | "past_due" | "incomplete" | "trialing" | "unpaid") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(subscriptions).set({ status }).where(eq(subscriptions.stripeSubscriptionId, stripeSubscriptionId));
}

export async function updateSubscriptionCancelAtPeriodEnd(stripeSubscriptionId: string, cancelAtPeriodEnd: boolean) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(subscriptions).set({
    cancelAtPeriodEnd,
...
export async function updateSubscriptionPlan(stripeSubscriptionId: string, plan: "starter" | "professional" | "enterprise", stripePriceId: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
